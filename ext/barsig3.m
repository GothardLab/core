...
% options.plotsig: 0=Don't plot any significance data
...
options = setdefaults(options,'plotsig',0,'diff',0,'mean',0,'trls',0,'group',0,'checktesttype',1,'labels',[],'errtype',1,'test_type','ranksum','color',[0.5 0.5 0.5],'plotsig',1,'siglvl',1,'stars',0,'a',0.05,'x',[1:ngroups],'cmap','nicejet','dotplot',1,'dotmsize',2,'dotcolor','k');
...
if options.plotsig
    if options.stars
        if options.group
            ...
            sigi=1;
            for k=1:ngroups
                if trls(k,1)~=0 && trls(k,2)~=0 && valz(k,1)>1 && valz(k,2)>1
            ...
            % Plot significance of within-group, between-level comparisons
