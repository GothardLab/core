...
if length(varargin)==0;
	...
	if length(size(Y))==2 
		...
		ind1=1:length(m);
		ind2=ind1(end:-1:1);
		hold on; h=fill([ind1 ind2],[m-s m(ind2)+s(ind2)],myColor);
		%set(h,'edgecolor',.6*ones(1,3))
        set(h,'edgecolor',myColor)
		plot(ind1,m,'color','k','color','k','linewidth',2)
		hold off
	elseif length(size(Y))>2 
		cla; hold on; 
		ind1=1:size(Y,2);
		ind2=ind1(end:-1:1);
		if size(Y,3)>8; col=myColor;ccol=myColor+.8; ccol(ccol>1)=1;end
		for k=1:size(Y,3)
			m=mean(Y(:,:,k));
			s=std(Y(:,:,k));
			h=fill([ind1 ind2],[m-s m(ind2)+s(ind2)],myColor);
			set(h,'edgecolor',myColor)
		end
		for k=1:size(Y,3)
			m=mean(Y(:,:,k));
			s=std(Y(:,:,k));
			plot(ind1,m,'color','k','linewidth',2,'color',myColor)
		end
		hold off 
	end
...
elseif length(varargin)==1;
	...
	elseif min(size(Y))==1;
		...
		hold on; h=fill([ind1 ind2],[m-s m(ind2)+s(ind2)],myColor);
		set(h,'edgecolor',myColor)
		plot(ind1,m,'color','k','linewidth',2)
		hold off
	else 
		ind1=(1:size(Y,1));
		ind2=ind1(end:-1:1);
		cla; hold on; 
		if size(Y,2)>8; myColor=jet(size(Y,2));myColor=myColor+.8; myColor(myColor>1)=1;end
		for k=1:size(Y,2)
			mm=m(:,k)';
			ss=s(:,k)';
			h=fill([ind1 ind2],[mm-ss mm(ind2)+ss(ind2)],myColor);
			set(h,'edgecolor',myColor)
		end
		for k=1:size(Y,2);
			mm=m(:,k)';
			ss=s(:,k)';
			plot(ind1,mm,'color','k','linewidth',2,'color',myColor)
		end
		hold off 
	end
end
