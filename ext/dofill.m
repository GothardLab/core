function [hgt h ysmth y x] = dofill(x,y,color,dim,smval,bars,filloff,transp,fs,frate,useSEM,fillOnly,rate,plotPoint)
...
if nargin < 14, plotPoint=0;end
if nargin < 13, rate =0;end
if nargin < 12, fillOnly = 0;end%If fillOnly=1, plot the fill but not the mean
...
if ~filloff
    if bars
        h = plot(x,ysmth,'color',color,'linewidth',lnwidth,'marker',markerstyle,'markersize',markersize);
        hold on;
        ...
        if transp
            ...
            %             whos vertices_y
            %             whos ysmth
            hh = fill(vertices_x,vertices_y, color, 'facealpha',.5, 'edgecolor',color,'edgealpha',0,'linewidth',eps);%edge must be 0 for multiple dudes on one plot
        else
    ...
    h = plot(x,ysmth,'color',color,'linewidth',lnwidth,'marker',markerstyle,'markersize',markersize);hold on;
end
